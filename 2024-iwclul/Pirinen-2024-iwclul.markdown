...
<span style='font-size:8pt'>(¹ Authors' archival version: Publisher’s version available at [ACL Anthology identifier: 2024.iwclul-1.16](https://aclanthology.org/2024.iwclul-1.16). All modern ACL conferences are open access usually CC BY)</span>
...
Our *research question* is, going from existing langauge technology
...
Our *key contributions* from this article are: *the experimental
framework* for others to compare and combine methods of gold data annotation for
smaller languages, the *pipelines* from traditional rule-based
...
product we have created, curated and annotated beginnings of *several new
...
1. rule-based generators and generative language models to generate a
starting point for a data set, to be proof-read and re-annotated by
humans,
1. rule-based analysers creating annotated dataset in legacy and ad hoc
formats that are converted and organised into a starting point for human
re-annotation, and
1. generative language models providing human annotators with starting
...
of *No language left behind* (NLLB) [(cites: nllb)](#nllb), also known for datasets
and evaluation schemes under *Scaling neural machine translation to next
200 languages* (FLoRES) [(cites: flores)](#flores). Unsurprisingly, this data set has so far
...
range of languages supported under the name of *next 1000
languages* [(cites: bapna2022buildingmachinetranslationsystems)](#bapna2022buildingmachinetranslationsystems). They have also
published several research papers listing exactly the sources they use to gather
information and data on the languages [(cites: ritchie-etal-2024-linguameta)](#ritchie-etal-2024-linguameta), this
...
if your tool works with the given language.  For example, the *Universal
Dependencies* (UD) treebanks [(cites: ud214)](#ud214), are used in a huge number of papers
...
*Unimorph* dataset, that concerns morphology of languages, has been used
...
source repository (footnote: <https://giellalt.github.io/lang-smn>).
...
the North Sámi [(cites: tyers-sheyanova-2017-annotation)](#tyers-sheyanova-2017-annotation) and Karelian
treebanks [(cites: pirinen2019building)](#pirinen2019building). For generation of the UniMorph data, some
of the datasets are generated based on rule-based generators [(cites: unimorph)](#unimorph),
...
source
licence [(cites: pirinen2023giellalt)](#pirinen2023giellalt). (footnote: <https://giellalt.github.io>)
...
UniMorph dataset generation task can be seen in Figure [(see: fig:chatuit)](#fig:chatuit).


**Figure:**
<!-- centering -->
![chatuit.png](chatuit.png)
(Caption: ChatGPT generating data for Inari Sámi UniMorph dataset.<a id="fig:chatuit">(¶ fig:chatuit)</a>)
<!-- end figure -->
...
`+N` to `N;` or `NOUN`. The mappings can also be 1:n or
...
containing freely available scripts. (footnote: <anonymised>) Some examples of
conversions are given in the Figure [(see: fig:conversions)](#fig:conversions).
...
(Caption: Conversions between traditional rule-based analyses and target dataset formats
    <a id="fig:conversions">(¶ fig:conversions)</a>)
<!-- end figure* -->
...
languages in the *No Languages Left Behind* and *Next Thousand
...
international corpora (SIKOR), [(cites: sikor_06.11.2018)](#sikor_06.11.2018) but we have also
...
discussed in the Section [(see: sec:discussion)](#sec:discussion).
...
table [(see: table:unimorphs)](#table:unimorphs).  The expected forms is based on the linguistic
grammars we have available [(cites: morottaja2023inarinsaamen)](#morottaja2023inarinsaamen).  We have measured
...
| ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| <!-- bf -->Nouns    |  58  |  100<span class='math'>^*</span>  |  100 %  |         |  14  |  15 %  |  21 %|
...
language model, Coverage and Accuracy measured in % units. <span class='math'>^*</span> Some extra
forms in rule-based model are due to allomorphy which was not accounted for
expected forms.<a id="table:unimorphs">(¶ table:unimorphs)</a>)
<!-- end table* -->
...
by [(cites: pirinen2019building)](#pirinen2019building), and asked LLM to generate similar hypotheses
likewise.  In Table [(see: table:ud)](#table:ud) we measure the post edit distance of the
sentences fixed and re-annotated, the error rates are calculated as
<span class='math'>E=\frac{S+I}{N}</span>, where <span class='math'>E</span> is the error rate, <span class='math'>S</span> is number of substitutions
made, <span class='math'>I</span> is the insertions made, and <span class='math'>N</span> number of readings (i.e. N is number
of CONLL-U lines with an index). We do not have <span class='math'>D</span> for deletions since both
...
(Caption: Caption<a id="table:ud">(¶ table:ud)</a>)
<!-- end table -->
...
editors today with a so-called *co-pilot*?
...
water  (footnote: <https://www.thetimes.com/uk/technology-uk/article/thirsty-chatgpt-uses-four-times-more-water-than-previously-thought-bc0pqswdr>)
and not insignificant amount of
energy [(cites: strubell2019energy)](#strubell2019energy). (footnote: <https://disconnect.blog/silicon-valley-is-sacrificing-the-climate-for-ai/>)
...
* <a id="nllb">**nllb**</a>:
    * title: No language left behind: Scaling human-centered machine tran...
    * author: Costa-jussà, Marta R and Cross, James and Çelebi, Onur and
    * journal: arXiv preprint arXiv:2207.04672
    * year: 2022
* <a id="flores">**flores**</a>:
...
* <a id="bapna2022buildingmachinetranslationsystems">**bapna2022buildingmachinetranslationsystems**</a>:
    * title: Building Machine Translation Systems for the Next Thousand L...
    * author: Ankur Bapna and Isaac Caswell and Julia Kreutzer and Orhan F...
    * year: 2022
    * eprint: 2205.03983
    * archivePrefix: arXiv
    * primaryClass: cs.CL
    * url: https://arxiv.org/abs/2205.03983
* <a id="ritchie-etal-2024-linguameta">**ritchie-etal-2024-linguameta**</a>:
...
* <a id="pirinen2019building">**pirinen2019building**</a>:
    ...
    * year: 2019
* <a id="unimorph">**unimorph**</a>:
    * title: UniMorph 4.0: Universal Morphology
    * author: Batsuren, Khuyagbaatar and Goldman, Omer and Khalifa, Salam ...
    * booktitle: Proceedings of the Thirteenth Language Resources and Evaluat...
    * pages: 840–855
    * year: 2022
...
url{http://gtweb.uit.no/korp}
    ...
    * year: 2021
* <a id="morottaja2023inarinsaamen">**morottaja2023inarinsaamen**</a>:

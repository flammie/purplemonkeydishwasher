...
<!-- usepackage amssymb  -->
<!-- set counter tocdepth 3 -->
<!-- usepackage url  -->

\newcommand{\keywords}[1]{\par\addvspace\baselineskip
<!-- no indent -->\keywordname\enspace\ignorespaces#1}
...
<span style='font-size:8pt'>(¹ Authors' archival version: This journal article was originally published in investigationes linguisticae <http://inveling.amu.edu.pl/index.php?page=issues&vol=21>.)</span>     
...
unigram probability training [(cites: pirinen2010finitestate)](#pirinen2010finitestate). With this probability
...
[(cites: wilcox-ohearn2008realword)](#wilcox-ohearn2008realword) are also possible.
...
[(cites: beesley2003finite)](#beesley2003finite), in practice lexc is a formalism for writing right linear
...
parallel rules as defined in the appendix of [(cites: beesley2003finite)](#beesley2003finite). We use
...
[(cites: pirinen2010finitestate)](#pirinen2010finitestate). 
...
<span class='math'>-\log \frac{f_w}{CS}</span>, where <span class='math'>f_w</span> is the frequency of the word and <span class='math'>CS</span>
the corpus size in number of word form tokens. For word-forms not appearing
in Wikipedia, we assign small probability by formula <span class='math'>-\log \frac{1}{CS+1}</span>.
...
algorithm [(cites: mohri2002efficient)](#mohri2002efficient). For automatic spelling corrections, the
...
wikipedia’s database backups (footnote: <http://download.wikimedia.org>). The
...
automata described in [(cites: beesley2003finitestate,linden2009hfst)](#beesley2003finitestate,linden2009hfst). The twolc rule
formalism is used for defining context-based rules with two-level automata and
they are described in [(cites: koskenniemi1983twolevel,linden2009hfst)](#koskenniemi1983twolevel,linden2009hfst).
...
been outlined in [(cites: beesley1998constraining)](#beesley1998constraining); the flag diacritics are denoted in lexc
...
1. FOR ∀ { {lines <span class='math'>morpheme/Conts</span> in dic}
1. <span class='math'>flags ← ε</span>
1. FOR ∀ { {<span class='math'>cont</span> in <span class='math'>Conts</span> <a id="a">(¶ a)</a>}
...
1. ENDFOR } <a id="b">(¶ b)</a>
1. FOR ∀ { {suffixes <span class='math'>lex, deletions, morpheme/Conts, context</span> in aff <a id="g">(¶ g)</a>}
1. <span class='math'>flags ← ε</span>
1. FOR ∀ { {<span class='math'>cont</span> in <span class='math'>Conts</span>}
1. <span class='math'>flags ← flags + *@C.cont@*</span>
1. <span class='math'>Lex_{Conts} ← Lex_{Conts} ∪*0 cont*</span>
1. ENDFOR }
1. <span class='math'>Lex_{lex} ← Lex_{lex} ∪*flags* + [&lt;lex] + *morpheme Conts*</span>
1. FOR ∀ { {<span class='math'>del</span> in <span class='math'>deletions</span>}
...
1. ENDFOR }
1. FOR ∀ { {prefixes <span class='math'>lex, deletions, morpheme/conts, context</span> in aff}
...
weighted edit distance transducer, see e.g.  [(cites: pirinen2010finitestate)](#pirinen2010finitestate)).  We
...
<!-- algoritmic -->[2]
1. FOR ∀ { {neighborsets <span class='math'>ns</span> in KEY <a id="vrb:pseudokeystart">(¶ vrb:pseudokeystart)</a>}
1. FOR ∀ { {character <span class='math'>c</span> in <span class='math'>ns</span>}
1. FOR ∀ { {character <span class='math'>d</span> in <span class='math'>ns</span> such that <span class='math'>c != d</span>}
...
1. FOR ∀ { {pairs <span class='math'>wrong, right</span> in REP <a id="vrb:pseudorepstart">(¶ vrb:pseudorepstart)</a>}
...
1. <span class='math'>w ← 0</span>
1. FOR ∀ { {character <span class='math'>c</span> in TRY <a id="vrb:pseudotrystart">(¶ vrb:pseudotrystart)</a>}
1. <span class='math'>w ← w + 1</span>
1. <span class='math'>Lex_{TRY} ← Lex_{TRY} ∪c:0_{&lt;w&gt;} \#  </span>
1. <span class='math'>Lex_{TRY} ← Lex_{TRY} ∪0:c_{&lt;w&gt;} \#  </span>
1. FOR ∀ { {character <span class='math'>d</span> in TRY such that <span class='math'>c != d</span>}
...
### Weighting Finite-State Dictionaries with Wikipedia
...
simply encodes a probability of a token appearing in the corpus [(cites: pirinen2010finitestate)](#pirinen2010finitestate).
The weights are formulated as penalty values belonging to the weighted semiring
using formula of <span class='math'>-\log\frac{f}{CS}</span> where <span class='math'>f</span> is the frequency of token, and
<span class='math'>CS</span> the size of corpus in tokens. For tokens not appearing in the corpus, a
maximum weight of <span class='math'>-\log\frac{1}{CS+1}</span> is used to ensure they will be suggested
...
tokenizing analysis algorithm[(cites: garrido-alenda/2002)](#garrido-alenda/2002). This algorithm uses
...
using the <span class='math'>-\log\frac{f}{CS}</span> formula for the weights, and this is unioned with
a version of original dictionary whose final weights have been set to the
maximum weight, <span class='math'>-\log\frac{1}{CS+1}</span>.
...
sites at the time of writing. The Table [(see: table:hunspell-automata)](#table:hunspell-automata) gives an
overview of the sizes of the compiled automata. The size is given in binary
multiples of bytes as reported by `ls -hl`.  In the
Table [(see: table:hunspell-automata)](#table:hunspell-automata), we also give the number of roots in the
...
The table [(see: table:error-analysis)](#table:error-analysis) summarizes the spelling suggestions made
...
hunspell errors.  The hunspell results were obtained by `hunspell -1 -d
\$LL &lt; misspelings`, and automata were applied using experimental HFST tool
`hfst-ospell error-model dictionary`. In the table, the column C is for
...
(Caption: Suggestion algorithm results)
  <a id="table:error-analysis">(¶ table:error-analysis)</a>
<!-- centering -->
  <div style='font-size: xx-small'>


| ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| **Language**  |  | <!-- FIXME: multicolumn 5 c -->{<!-- bf -->Hunspell}  |  | <!-- FIXME: multicolumn 5 c -->{<!-- bf -->FST}  |  | <!-- FIXME: multicolumn 5 c -->{<!-- bf -->FST + Unigrams}  |  | <!-- FIXME: multicolumn 5 c -->{<!-- bf -->FST + 4 errors}  |
...
AMD 8360 and 512 GiB of RAM memory available 
 (footnote: <http://www.csc.fi/english/pages/hippu\_guide/introduction/overview/index\_html/?searchterm=hippu>).
...
* <a id="beesley1998constraining">**beesley1998constraining**</a>:
    * author: Beesley, Kenneth R.
    * title: Constraining separated morphotactic dependencies in finite-s...
    * year: 1998
    * isbn: 975-7679-34-8
    * pages: 118–127
...
* <a id="garrido-alenda/2002">**garrido-alenda/2002**</a>:
    * error: <strong style="color: red">missing from bibs</strong>

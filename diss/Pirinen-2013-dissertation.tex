...
\author{Tommi A \misspelt{Pirinen}}
...
\pubpages{98} % --- remember to update this!
...
\custos{Prof. Arto Mustajoki, University of Helsinki, Finland}
...
\begin{abstract} 
    ...
    \textfinnish{
    \input{Pirinen-2013-dissertation-abstrakti.tex}
    }
\end{abstract}
...
\newglossaryentry{language model}{
    ...
    description={is any software implementation or formal language capable of
        ...
        of spell-checking or not. Furthermore may contain additional
        information of the word-form, such as probability of being correct or
        linguistic information. This usage differs slightly from established
        usage in natural language engineering, where language model refers to
        a statistical \(n\)-gram model, which is just a one sub-class of
    my usage}
}
...
\newglossaryentry{morphology}{
    name={morphology},
    description={is a branch of linguistics studying word-formation, structure
    of word-forms}
}
...
\newglossaryentry{morphological complexity}{
    ...
    description={is used in this dissertation in very limited sense of
        complexity of a language, that is relevant to spell-checking and correction and
        manifested in terms of amount and predictability of word-forms the
    language contains}
}
...
\newglossaryentry{grammar-checker}{
    ...
    and their combinations, and their relation to whole sentence}
}

\newglossaryentry{spell-checking}{
    name={spell-checking},
    description={is the task of verifying word-forms of the text are correctly
    written word-forms in the language of spell-checker. Spell-checker
...
\newglossaryentry{spelling correction}{
    ...
    description={is the task of correcting misspelled word-forms in the text
        by correct ones, or suggesting alternatives in an inter-active
    application}
}
...
\Gls{spell-checking} and \glslink{spelling correction}{correction} is among the
...
\glslink{morphological complexity}{morphologically complex} languages around the
...
simple approaches, we come to the second part of the thesis topic:
finite\-/state methods. In this dissertation I have selected to use
\glspl{finite-state automaton} as a general framework for handling of the
spell-checking dictionaries as a main part because of its success in handling
of morphologically varied languages~\cite{beesley2003finite}. This approach is
sufficient to cover the infinite dictionaries of the kind mentioned in the
previous section.  It is commonly thought that the extents of finite-state
technologies are sufficient to cover most of the natural languages' word-forms
found in the running texts, that is, a spell-checker's dictionary implemented
with these technologies could know all correctly written word-forms without
technology being the limiting factor. There are some counter-arguments to this,
e.g., \citet{culy1987complexity} presents features of Bambara vocabulary that
would require expressive powers beyond regular and even context-free grammars
to predict Bambara's word-forms. To me it seems that there are two limitations
to this specification that allow finite-state approximations of Bambaran
...
handled on vocabulary side; that already relieves finite-state spell-checkers
from the burden of recognising such lexical elements due to the fact that
finite-state spell-checkers are forced to operate on space-separated strings.
This definition does allow for Bambara to have ungrammatical forms in
spell-checked text, such as \misspelt{\emph{wulunyina o wulufilela}} (dog
searcher dog watcher) mentioned in the text. This, however is akin to
spell-checker allowing English phrase \misspelt{\emph{I sees he}} or Finnish
\misspelt{\emph{punaiselle kukasta}} (to red from flower), something that in
contemporary software is relayed from spell-checking to grammar checking. The
second saving aspect for finite-state technology is the capability of it to
describe all finite subsets of languages higher in the hierarchy. What it means
is that, should there be a language having productive word-formation like
Bambara without intervening white spaces, it is still possible to capture the
...
spell\-/checking system.  Specifically in \citepalias{pirinen2010creating} I
...
\citepalias{pirinen2012effects,pirinen2013quality} we have already reached the
...
traditional simple word-list spell\-/checking models to \glslink{morphological complexity}{complex morphological}
...
complexity}{morphologically complex} languages with lesser resources. For this
...
error correction was introduced in~\citepalias{pirinen2010finitestate}, but the
...
working with more \glslink{morphological complexity}{morphologically complex}

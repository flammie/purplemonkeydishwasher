...
master's thesis work, which is modeling of the Finnish language
efficiently---using this language model as a part of practical, end user
software, and towards advancement of scientific treatment of the problems when
doing so, it was very obvious and motivating development for a doctoral
...
because I believe the productivisation and social engineering required to make
...
Greenland, whose native language we all know is in fact trickier to handle than
most of ours. As the results of the testing were not at all so negative, we can
hope that future does bring better spell-checkers and autocorrections for us
with the slightly more tricky native languages.
...
%%machines\footnote{\url{http://en.wikipedia.org/wiki/FSM}}---without them,
...
articles \citepalias{pirinen2009weighted,pirinen2009weighting} we explore some
...
languages. In \citepalias{pirinen2010finitestate} we try to tackle the problem
with scarcity of the corpus resources especially in conjunction with
...
spell-checking results. In \citepalias{pirinen2010building,pirinen2010creating}
and then again \citepalias{pirinen2012compiling} I have researched the
compilation and use of existing non-finite state language descriptions as
finite-state automata language models of finite-state spell-checking systems.
Specifically in \citepalias{pirinen2010creating} I attempt to recreate much of
Hunspell spell-checking system, including the error correction part, in
finite-state form. In \citepalias{pirinen2011modularisation} I research the
topic of maintenance and upkeep of finite-state language models, and try to
prove why finite-state language models are feasible for long-term maintenance
for vast array of applications with just one language description instead of
one per application.  In \citepalias{pirinen2012improving} I tackled the
problematic issue of context-aware spell-checkers for morphologically complex
and resource-poor languages, showing and proving what practical limitations are
and what can be done to get some benefits of such implementation. In
...
%after all.
The chronological order is also visible in
the schematic diagram of figure~\ref{fig:schematic-diagram}: the finite-state language
models (in the leftmost package) were conceptualised after the statistical
weighting schemes of morphologically complex language (in the middle package)
and the resulting combinations were evaluated on larger scale.
...
of the Finnish finite-state implementation of morphological analyser we were
...
evaluation scheme and major parts of the article are work of Krister Lindén.
In~\citepalias{pirinen2010finitestate} I already take into account that
previous experiments were performed using commercial corpora unavailable for
scientific, free and open source work, which lead me to devise the statistic
training as well as the evaluation suite based on the freely available
...
programming. The engineering work on Hunspell language
models~\citepalias{pirinen2010building,pirinen2010creating} originated from the
...
finite-state methods of \citepalias{pirinen2012improving} were originated
from~\citet{silfverberg2010partofspeech}, with whom I also co-operated in
porting the system to finite-state spelling checker use. The evaluation scheme
was slightly modified from earlier one used in
\citepalias{pirinen2010building}. The writing of the article
\citepalias{pirinen2012improving} was mainly my work, whereas the mathematical
formulas were build in co-operation by article's authors.
In~\citepalias{pirinen2012effects}, the evaluation setup and article was
written by myself, with majority of the technical implementations done by Sam
Hardwick, who is also the main author behind the finite-state spell-checking
component of our current software, as is further detailed
in~\citep{linden2011hfst}.  In the
...
only. It is, however, true that all of the work within this thesis is performed
in part in the HFST research group, and there is no doubt that there is an
abundance of minor ideas regarding to the engineering of the software,
evaluation setup and article ideas that are creation of collective
...
The rest of the thesis is organised by topic into chapters as follows: In
...
bring statistics to the language and error models. In~\ref{chap:error-models} I
study the finite-state formulations of error modeling and contrast my
...
efficient Knuth-foo-bar\todo{check and cite} string matching algorithm, it is
...
While computation power and storage space has quite raised along Moore's
...
approach in one of the articles as well, for Finnish.
...
familiar to anyone who has dabbled in weighted finite-state theory e.g. from
...
more times than they would have been seen in reality. 
...
have presented~\citep{linden2009hfst,pirinen2010building}---I suggest this
...
manually annotated data I have studied into salvaging what can be easily used
...
language models for spell-checking. This branch of research would have started
...
give significant gains to spell checking language models when properly coupled
with rule-based language models. I have hypothesised that the underlying reason
for this is that the events where simple spell-checking like edit distance or
confusion sets produce ambiguous results at same length, in majority of the
cases it is beneficial to vote for the more common word-form.
...
\textbf{Main Article}: \emph{Building and Using Existing Hunspell Dictionaries
and \TeX\ Hyphenators as Finite-State Automata} by myself and Krister Lindén.
In this article I have formulated the Hunspell's software based algorithms for
...
of all the language and error models and testing approaches I have developed in
